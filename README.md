...
**全球颱風動態**
- 日本氣象廳（JMA）颱風情報：西太平洋現役颱風的結構化實況（位置、氣壓、風速、路徑），
  對台灣最重要，多來源重複時以此為準。
- 美國國家颶風中心（NHC）：大西洋／東太平洋現役系統。
- 美軍聯合颱風警報中心（JTWC）：補南半球與印度洋等其他海域。
- 強度以中央氣象署分級稱呼（輕度／中度／強烈颱風），括號附國際分類；
  西太平洋系統會說明對台灣的潛在影響。颱風是每日更新的狀態，
  以「系統＋日期」去重，活躍期間每天都會出現在日報中。
...
**論壇形式**：Webhook 指向 Discord「論壇頻道」時，每天的日報自動成為一篇獨立貼文
（標題如「🌍 地球科學日報 2026-07-16（週四）」），焦點地震圖解會接在同一篇裡，
週日回顧另開一篇——頻道不會被訊息流洗版。若 Webhook 指向一般文字頻道，
會自動退回逐則訊息模式。

**閱讀份量**：目標是 15 分鐘的完整晨讀（正文約 3500~5000 字），每則都是有頭有尾的
小篇新聞（背景 → 發生什麼 → 所以呢），深度導讀是 600~900 字的主菜文章，
不是條列式摘要。風格參考泛科學（PanSci）。每天的內容：

1. **🌍 導言卡**（湖水綠）：英中雙語的今日看點。
2. **🇹🇼 台灣地震動態**（琥珀）：台灣及周邊 M4+ 地震，平靜時會直接告訴你「台灣平安」。
2-1. **🌀 全球颱風動態**（青）：JMA/NHC/JTWC 的現役熱帶氣旋合併摘要，西太平洋優先。
3. **🌋 地球物理與地震學**（紅）：主要焦點。每則＝英文標題＋英文摘要（練英語）＋
   中文 2~3 句重點（發生什麼＋意義）＋ 💡 科普補充（背後原理或背景知識）＋來源＋連結。
4. **🔭 其他地球科學**（藍）：次要補充，格式同上。
5. **🔬 今日深度導讀**（綠）：每天挑最有意思的一則寫成 250~350 字迷你科普文——
   生活化開場、背景脈絡、新發現、為什麼重要、結尾一個 fun fact 或思考點。
6. **📖 每日一詞**（紫）：從當天內容挑一個專業術語，附英文定義、學術例句、台灣譯名
   與記憶點；已教過的詞會記錄在 `data/words.json`，90 天內不重複。

內容太長時會自動拆成多則 Discord 訊息（單則訊息有 6000 字元上限）。

## 每日焦點地震圖解（自動生成圖片）

`scripts/quakeplot.py` 每天挑出最值得看的一起地震（台灣周邊 M4.5+ 優先，否則全球
規模最大的 M5.8+；都沒有就自動跳過），生成兩張圖推播：

1. **🌊 測站記錄剖面（record section / moveout）**：用 ObsPy 從 IRIS/EarthScope
   抓全球開放測站（IU/II 網、台灣事件加抓中研院 BATS）的真實垂直向波形，
   按震央距排列，疊上 iasp91 模型的 P、S 理論走時曲線。附「怎麼看這張圖」教學說明。
   會自動剔除斷訊或訊噪比不足的測站，每個距離區間遞補候選測站。
2. **🗺️ USGS ShakeMap 震度圖**：官方的地表震動強度（MMI）分布圖，直接取自
   USGS 事件產品，非自行估計。

門檻可調：`quakeplot.py` 開頭的 `TAIWAN_MIN_MAG` 與 `GLOBAL_MIN_MAG`。
本地測試：不設 `DISCORD_WEBHOOK_URL` 直接執行，會把圖存到當前目錄並印出訊息內容。
...
## 台灣地震報告（事件驅動，需要 CWA_API_KEY）

`scripts/quakereport.py` + `.github/workflows/quake-report.yml` 是獨立於每日日報的
事件驅動機器人：每 15 分鐘輪詢中央氣象署「顯著有感地震報告」(E-A0015-001) 與
「小區域有感地震報告」(E-A0016-001)，一有新的**正式地震報告**（非速報）就彙整推播到
專屬的 Discord 論壇，每起地震一篇貼文，內容包含：

- 發震時間、震央、經緯度、深度、規模、最大震度
- 各縣市最大震度（依震度排序，附該縣市最大測站）
- 氣象署官方地震報告圖與等震圖
- 一段 AI 科普解說（地體構造背景、深度與規模的意義；純解說，不含防災指示）

**⚠️ 這不是地震預警系統**：用的是地震後數分鐘經人工審定的正式報告，加上輪詢與
GitHub 排程延遲，訊息約在地震後 15~40 分鐘送達。即時避難請以氣象署的警報服務為準。

**需要的 secrets**：`CWA_API_KEY`（見下方申請說明）、`DISCORD_WEBHOOK_QUAKE`（地震報告論壇）。
推播門檻可調：`quakereport.py` 開頭的 `MIN_MAGNITUDE`（預設 4.0）與 `MIN_INTENSITY_RANK`。

### 申請 CWA_API_KEY（免費）
1. 到[氣象資料開放平臺](https://opendata.cwa.gov.tw/)註冊會員（免費）。
2. 登入後在「會員資訊 → API 授權碼」取得一組 `CWA-` 開頭的授權碼。
3. 到 repo 的 Settings → Secrets 新增 `CWA_API_KEY`。這組金鑰同時也會讓每日日報的
   台灣地震專區升級成氣象署資料。
